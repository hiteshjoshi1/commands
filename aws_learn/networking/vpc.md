...
1. When we create a VPC, a  default network ACL is created. This deafulat NACL allows all in bound and outbound traffic.
2. Everytime a subnet is created , it is added to default network ACL. An association to a network ACL is mandatory for a subnet so if you do not explicitly associate with a NACL, the subnet will get associated with default NACL.
3. You can create a new ACL and then change subnet association from default ACL to your newly created ACL. This newly created NACL will have all in bound and outbound traffic blocked.
4. **A subnet can only be associated to one ACL at a time.  So when you add a subnet to your new ACL , it's previous association will be removed from the previous network ACL.
5. **An ACL can however can associate with multiple subnets which it governs.
6. NACL are state less, that is seprate in bound and outbound rules have to be created. Unlike Sec Groups which are stateful, an inbound SG rule will automatically create an outbound SG rule.
7. **Rules are evaluated top down - from low to High**. So if you have a deny rule , make sure that its rule # is less than a Allow Rule.
9. **Network ACL act before Security groups. So if anything is denied at network ACL, it will never reach Security group.
9. By default , in a new network ACL's - it blocks all inbound and outbound traffic.
10. Network ACL are stateless, need to add specific inbound and outbound rules seprately(Unlike Sec groups).
...
Suppose you are creating a Application Load balancers in the custom VPC you created above. You select it and the select subnet, if you select private subnet, it will show you warning saying - Creating an internet facing load balancer but there is no Internet gateway attached to the subnet you selected.
If however, you choose the public subnet, you get an error -- Atleast 2 public subnets need to specified. 
**So atleast 2 public subnets are required to create a Load Balancer.



### VPC Flow Logs
- VPC flow logs is a feature that enables you to capture IP traffic going in and out of your network interface to your VPC.
- Flow data is stored in AWS Cloudwatch Logs and can be viewed from there.
- You cannot enable flow logs for VPCs that are peered with your VPC unless he Peer VPC is in your account.
- Can be created at 3 levels -
  - VPC
  - Subnet
  - Network interface level
4. cannot create a flow log for VPC's peered with your VPC unless that VPC is also within your a/c.
5. Can tag a flow log
...
### Direct Connect
Dedicated network Connection between your data center to AWS. Better private connectivity between AWS and your data center.
Userful high throughput workloads. 
Stable or reliable secure connection.

#### Set Up Direct Connect
- Create a virtual interface
- go to VPC console then to VPC ocnnections. Create Customer gateway
- Create a Virtual Private Gateway
- Attach the Virtual private gateway to desired VPC
- Select the VPN connections and create new VPN connection
- Select the Virtual private gateway and customer gateway
- once the VPN is availaible , set up the VPN on the customer gateway or firewall
